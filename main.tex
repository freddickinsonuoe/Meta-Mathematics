...
\end{equation*}
Thus, in order for $t$ to be rational, we require $-3p = n^2$ for some integer $n \in \N_+$. With the restriction that $p$ is an integer itself, we need $-3p = m^2$ for some integer $m \in \{3n : n \in \N\}$. Examples of $p$ include
\begin{equation*}
    p \in \{-3, -12, -27, -48, \dots\}.
\end{equation*}
\hl{Will add the code and examples here.} Including the desmos.
